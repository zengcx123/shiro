					...
					<div class="btn-group m-r">
						<button data-toggle="dropdown"
							...
							<span class="dropdown-label"><c:if
									test="${user.locked eq 1}">正常</c:if>
								<c:if test="${user.locked eq 0}">禁用</c:if></span> <span class="caret"></span>
						</button>
						<ul class="dropdown-menu dropdown-select">
							<li class="active"><a href="#"><input type="radio"
									name="userFormMap.locked" value="1"
									<c:if test="${user.locked eq 1}"> checked="checked"</c:if>>正常</a></li>
							<li class=""><a href="#"><input type="radio"
									name="userFormMap.locked" value="0"
									<c:if test="${user.locked eq 0}"> checked="checked"</c:if>>禁用</a></li>
						</ul>
					</div>
				</div>
			</div>
