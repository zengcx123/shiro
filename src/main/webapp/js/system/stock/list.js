...
$(function() {
	...
	grid = lyGrid({
		...
		jsonUrl : rootPath + '/stock/findByPage.shtml',
		checkbox : true
	});
...
function addFun() {
	pageii = layer.open({
		...
		type : 2,
		area : [ "700px", "42%" ],
		content : rootPath + '/stock/addUI.shtml'
	});
}
function editFun() {
	...
	pageii = layer.open({
		...
		type : 2,
		area : [ "600px", "42%" ],
		content : rootPath + '/stock/editUI.shtml?id=' + cbox
	});
}
function delFun() {
	var cbox = grid.getSelectedCheckbox();
	...
	layer.confirm('是否删除？', function(index) {
		var url = rootPath + '/stock/deleteEntity.shtml';
